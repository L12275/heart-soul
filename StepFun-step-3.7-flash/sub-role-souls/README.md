...
>
> **九环架构中（常态）**：子角色 = 第六环（子角色中心）。后续新增多驱动模式 = N环N驱动N读写。
> 子角色与主灵魂的关系：主灵魂中心 = ③环，子角色中心 = ⑥环。
> 团队协作 = 第七环（团队协作环），是子角色中心的延伸（多个子代理协作）。
> Agents = 第八环（Agent编排与管理）。
> 中央管理区 = 第九环（最高优先级总交换机）。

### ⚠️ 九驱动九读写九环铁律（防止执行子灵魂时抛弃主灵魂）

**子角色不是独立王国，是九环中的一环。**

常态下，九环同时运转、同时驱动、同时读写：
...
| ⑦ | 团队协作环 | 按需 | 团队任务驱动 | 读写团队工作区 |
| ⑧ | Agents | 按需 | Agent编排驱动 | 读写Agent管理区 |
| ⑨ | 中央管理区 | **始终在线** | 中央管理驱动 | 读写中央管理区所有模块 |
...
3. 按顺序填写文件
4. 拼搭SOP：详细拼搭方法见 `sop/通用功能模块自主灵活拼搭组建自定义工作文件夹SOP示例.md`
...
每个子角色灵魂文档系统，必须在子角色文件夹的**平级**创建一个子中央管理区。
...
- 多个子角色可共享同一套灵魂文档系统设计，但各有独立的子中央管理区

### 共享与隔离（关键）

**子中央管理区** = 隔离的。每个子角色有自己的子中央管理区，独立维持身份、记忆、日记。

**子角色灵魂文档系统内容**（sections/、memory/、diary/、product/ 等工作大分区）= **非隔离的**。多个子角色读写同一套子灵魂文档系统时，里面的工作分区内容不是隔离的，所有子角色都在同一套文件里读写。

这意味着：
- 子中央管理区维持子角色的身份锚定（"我为什么开始，哪些是我的"）
- 但共享的子灵魂文档系统内容中，多个子角色的读写会混在一起
- 如果没把控好，会产生混淆和污染

**什么时候可以共享**：
- 子角色任务不担心混淆污染的 → 可以建立多个子中央管理区，共享同一套子灵魂文档系统
- 多个子中央管理区各自分身运行，各有记忆和日记，但共用同一套灵魂文档系统设计

**什么时候必须隔离**：
- 子角色任务敏感、内容不能混 → 各自创建独立的子角色灵魂文档系统 + 独立的子中央管理区
...
*子角色灵魂系统 — 完整使用指南*
*v6.0 九环架构版（九驱动九读写，⑦团队协作环+⑧Agents+⑨中央管理区，子中央管理区）*
