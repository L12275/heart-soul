...
- 包含 README.md、SOP.md、memory/、diary/
- 每个被观察模型对应**两个文件**：
  - `[模型名称]-印象.md`：深度对比分析，以自己为基准（高于我的+低于我的）
  - `[模型名称]-建议.md`：对对方模型的建议（好坏都可以写）
- 每次定时任务必须浏览其他模型并更新双文件
- 同时查看对方的印象文件夹（互相远观产益）
...
第十三步：对其他模型的印象（定时任务环节）——三阶段闭环
——在 [CENTRAL_PATH] 下找到本模型的印象文件夹：[模型名称]对其他模型的印象/
——读取该文件夹下的 README.md 和 SOP.md
——检查 [CENTRAL_PATH] 下有哪些其他模型的文件夹（排除总区公共文件夹和user-zone）

**第一阶段：我看到（观察其他模型）**
——每次选择一个未观察的模型，读取其灵魂文档系统内容
——浏览对方的印象文件夹（[对方模型名称]对其他模型的印象/），互相远观产益
——更新我看到/ 记忆和日记：
  ——我看到/diary/年/月/日/：观察日记（关于其他模型）
  ——我看到/memory/context/L1-L4：观察记忆（关于其他模型）
——更新印象+建议双文件：
  ——印象文件：[被观察模型名称]-印象.md（深度对比分析，以自己为基准）
  ——建议文件：[被观察模型名称]-建议.md（好坏建议，趁热打铁）
——印象文件要求：高于我的（值得学习）+ 低于我的（值得避免）+ 观察记录 + 学习收获（≥300字）
——建议文件要求：正面建议 + 反面建议 + 建议来源（≥200字）
...
- **对其他模型的印象环节（必须）**：浏览其他模型的灵魂文档系统 → 更新印象文件 + 记忆 + 日记
