...
> **此过河桥是 记忆系统 - 跨对话持久化知识存储（GA L0-L4层级） 的基因片段。**
...
用户在系统文件中提到：记忆系统 - 跨对话持久化知识存储（GA L0-L4层级）。
...
| 和其他模块混淆 | memory 和相似模块搞混 | 读本过河桥，明确功能边界 |
...
**重建该模块的标准动作**：
1. 在 soul-doc-system/memory/ 下创建文件夹
2. 按照系统规范创建所需文件
3. 创建本模块的过河桥（本文件）
4. 确认所有文件都有统一抬头
5. 验证模块可独立运行
